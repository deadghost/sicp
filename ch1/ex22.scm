...
(define (search-for-primes x y)
  (define (prime-list x y cnt)
    (cond ((or (= x y) (= cnt 3)) nil)
          ((prime? x)
           (cons x (prime-list (+ x 1) y (+ cnt 1))))
          (else (prime-list (+ x 1) y cnt))))
  (prime-list x y 0))

(define (start-search-for-primes-test x y start-time)
  (if (search-for-primes x y)
      (display (- (runtime) start-time))))

(define (timed-search-for-primes x y)
  (newline) (start-search-for-primes-test x y (runtime)))

(timed-search-for-primes 1000 10000) ; => 49
(timed-search-for-primes 10000 100000) ; => 133
(timed-search-for-primes 100000 1000000) ; => 309

;; Yep timing data is consistent with predications.
